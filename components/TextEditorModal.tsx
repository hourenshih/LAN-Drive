        ...
        <main className="flex-1 min-h-0 relative">
          {isLoading ? (
            ...
            <textarea
              ...
              className="w-full h-full p-4 resize-none border-0 focus:ring-0 dark:bg-gray-800 dark:text-gray-200 font-mono text-sm leading-6"
              spellCheck="false"
            />
          )}
        </main>
        
        <footer className="bg-gray-50 dark:bg-gray-700 px-6 py-3 flex justify-between items-center rounded-b-lg flex-shrink-0">
            {error && !isSaving && <p className="text-sm text-red-600 truncate max-w-xs">{error}</p>}
            {hasUnsavedChanges && !error && <p className="text-sm text-yellow-600 dark:text-yellow-400">You have unsaved changes.</p>}
